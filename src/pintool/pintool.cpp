...
// List of important addresses loaded during recovery
std::unordered_map<uintptr_t, uintptr_t> impaddrs;
...
KNOB<BOOL> KnobRecovery(KNOB_MODE_WRITEONCE,  "pintool",
        "rec", "0", "specify profile in recovery mode ");
...
void roi_handler(string func_name) {
    ...
    else if( collect_impaddr && func_name == "cpavise_tx_begin"){
        collect_impaddr = false;
        for(auto it = impaddrs.begin(); it != impaddrs.end(); it++){
            //PinDEBUG(*out << it->first << "\n";);
        }
        //PinDEBUG(*out << "ImpAddr collection ends\n\n";);
    }
}

void memfunc_handler(UINT32 id, ADDRINT arg0, ADDRINT arg1, ADDRINT arg2) {
    //this return makes some small difference?
    //return;
    if(!in_roi || !record_memfunc || recmode) return;
    ...
    if(!info.visited) {
        ...
        if(info.line){
            if (WITHIN_PMEM(arg0) || WITHIN_PART(arg0)){
                ...
                PinDEBUG(*out << path << ":" << info.line << endl;);
...
void record_pmread (ADDRINT ip, void* addr) {
    if(recmode){
        if ( collect_impaddr && (WITHIN_PMEM(addr) || WITHIN_PART(addr)) ){
            struct ins_info info = ins_addr2info[ip];
            ...
            if(info.line){
                string path = get_abs_path(info.path);
                impaddrs.insert({(uintptr_t)addr,0});
                if(!info.visited){
                    PinDEBUG(*out << path << ":" << info.line << endl;);
                    ins_addr2info[ip].visited = true;
                }
        ...
        else if(WITHIN_PMEM(addr) || WITHIN_PART(addr)){
            ...
            if(!info.visited) {
                if(info.line){
                    string path = get_abs_path(info.path);
                    if(impaddrs.find((uintptr_t)addr) != impaddrs.end()){
                        //PinDEBUG(*out << "IMP PM READ at addr: " << std::hex << addr << std::dec << endl;);
                        PinDEBUG(*out << path << ":" << info.line << endl;);
                        ins_addr2info[ip].visited = true;
                    }
                }
        ...
        if (WITHIN_PMEM(addr) || WITHIN_PART(addr)){
...
void record_pmwrite (ADDRINT ip, void* addr) {
    ...
    if(0){
        if(collect_impaddr && (WITHIN_PMEM(addr) || WITHIN_PART(addr))){
            ...
            if(!info.visited) {
                if(info.line){
                    string path = get_abs_path(info.path);
                    if(impaddrs.find((uintptr_t)addr) != impaddrs.end()){
                        //PinDEBUG(*out << "IMP PM WRITE at addr: " << std::hex << addr << std::dec << endl;);
                        PinDEBUG(*out << path << ":" << info.line << endl;);
                        ins_addr2info[ip].visited = true;
                    }
                }
        ...
        if (WITHIN_PMEM(addr) || WITHIN_PART(addr)){
            struct ins_info info = ins_addr2info[ip];
            if(!info.visited) {
                //PinDEBUG(*out << "PM WRITE at addr: " << std::hex << addr << std::dec << endl;);
                //PinDEBUG(*out << "INS Mnemonic: " << info.mnemonic << endl;);
                //PinDEBUG(*out << "INS Disassemble: " << info.disassemble << endl;);
...
VOID Routine(RTN rtn, VOID* v){
    ...
    if(name.find("cpavise_tx_begin") != string::npos) {
        RTN_InsertCall(
...
int main(int argc, char *argv[])
{
    ...
    recmode = KnobRecovery.Value();
    if(recmode)
        cerr << "Pintool: recmode = " << recmode << endl;
