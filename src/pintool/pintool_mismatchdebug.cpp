...
#define WITHIN_PMEM(a) WITHIN_SHADOW(a) //((uintptr_t)a >= PMEM_BASE && (uintptr_t)a < SHADOW_BASE)
...
bool check_memxxx(string name) {
    if(name.find("pmem") != string::npos || name.find("pavise") != string::npos){
        return false;
    }
...
VOID Routine(RTN rtn, VOID* v){
    ...
    if(rc->_name[0] != '_'){//&& rc->_name.find("plt") == string::npos){
        RTN_InsertCall(
