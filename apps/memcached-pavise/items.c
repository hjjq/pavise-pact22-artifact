...
item *do_item_alloc(char *key, const size_t nkey, const unsigned int flags,
    ...
    it->nkey = nkey; //if((uint64_t)it >= 0x30000000000ULL) printf("it->nkey = %p\n", (void*)&(it->nkey));
    it->nbytes = nbytes;
    memcpy(ITEM_key(it), key, nkey);//printf("memcpy: dest = %p\n", (void*)ITEM_key(it));
    it->exptime = exptime;
    if (settings.inline_ascii_response) {
        memcpy(ITEM_suffix(it), suffix, (size_t)nsuffix);//printf("2memcpy: dest = %p\n", (void*)ITEM_suffix(it));
    } else if (nsuffix > 0) {
        memcpy(ITEM_suffix(it), &flags, sizeof(flags));//printf("3memcpy: dest = %p\n", (void*)ITEM_suffix(it));
    }
    it->nsuffix = nsuffix;
...
int do_item_link(item *it, const uint32_t hv) {
    ...
    if (it->it_flags & ITEM_PSLAB) {
        ...
        it->it_flags |= ITEM_LINKED;//if((uint64_t)it >= 0x30000000000ULL) printf("it->it_flags = %p\n", (void*)&(it->it_flags));
    ...
    item_link_q(it);
    refcount_incr(it);
    item_stats_sizes_add(it);//if((uint64_t)it >= 0x30000000000ULL) printf("it= %p\n", (void*)(it));
...
void do_item_unlink(item *it, const uint32_t hv) {
    ...
    if ((it->it_flags & ITEM_LINKED) != 0) {
        it->it_flags &= ~ITEM_LINKED;//if((uint64_t)it >= 0x30000000000ULL) printf("it->it_flags550 = %p\n", (void*)&(it->it_flags));
#ifdef PSLAB
        if (it->it_flags & ITEM_PSLAB)
            pmem_member_persist(it, it_flags);
#endif
...
void do_item_remove(item *it) {
    ...
    if (refcount_decr(it) == 0) {//if((uint64_t)it >= 0x30000000000ULL) printf("it591 = %p\n", (void*)(it));
        item_free(it);
    }
}
...
char *item_cachedump(const unsigned int slabs_clsid, const unsigned int limit, unsigned int *bytes) {
    ...
    while (it != NULL && (limit == 0 || shown < limit)) {
        ...
        if (bufcurr + len + 6 > memlimit)  /* 6 is END\r\n\0 */
            break;
        memcpy(buffer + bufcurr, temp, len);//printf("4memcpy: dest = %p\n", (void*)(buffer+bufcurr));
        bufcurr += len;
        shown++;
        it = it->next;
    }

    memcpy(buffer + bufcurr, "END\r\n", 6);//printf("5memcpy: dest = %p\n", (void*) (buffer+bufcurr));
    bufcurr += 5;
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv, conn *c, const bool do_update) {
    ...
    if (it != NULL) {
        refcount_incr(it);//if((uint64_t)it >= 0x30000000000ULL) printf("it1023 = %p\n", (void*)(it));
