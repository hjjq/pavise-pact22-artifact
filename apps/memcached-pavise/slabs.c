...
static void *do_slabs_alloc(const size_t size, unsigned int id, uint64_t *total_bytes,
    ...
    if (p->sl_curr != 0) {
        ...
        it->refcount = 1;//if((uint64_t)it >= 0x30000000000ULL) printf("it->refcount = %p\n", (void*)&(it->refcount));
        p->sl_curr--;//if((uint64_t)p >= 0x30000000000ULL) printf("p->sl_curr addr = %p\n", (void*)&(p->sl_curr));
        ret = (void *)it;
    } else {
        ret = NULL;
    }
...
#endif
        it->it_flags = ITEM_SLABBED;//if((uint64_t)it >= 0x30000000000ULL) printf("it->it_flags = %p\n", (void*)&(it->it_flags));
#ifdef PSLAB
...
#endif
        it->slabs_clsid = 0;//if((uint64_t)it >= 0x30000000000ULL) printf("it->slabs_clsid = %p\n", (void*)&(it->slabs_clsid));
        it->prev = 0;
        it->next = p->slots;
        if (it->next) it->next->prev = it;
        p->slots = it;
