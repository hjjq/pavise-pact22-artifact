...
void item_init(bool recover, PMEMobjpool *pop) {
    int i;
    TOID(struct lru_lists)* toid_lists;
    if (recover) {
        *toid_lists = POBJ_FIRST(pop, struct lru_lists);
        heads = D_RW(*toid_lists)->heads;
        tails = D_RW(*toid_lists)->tails;
        sizes = D_RW(*toid_lists)->sizes;
    } else {
        POBJ_NEW(pop, toid_lists, struct lru_lists, NULL, NULL);
        TOID(item**) toid_heads;
        TOID(item**) toid_tails;
        TOID(unsigned int*) toid_sizes;
        POBJ_ZALLOC(pop, 
    }
        heads = (item**) nvm_reserve_id("heads", LARGEST_ID * sizeof(item*));
        for(i = 0; i < LARGEST_ID; i++) {
            heads[i] = NULL;
        }
        clwb_range(heads, LARGEST_ID *sizeof(item*));
        sfence();
        nvm_activate_id("heads");
    } 
    if (!tails) {
        tails = (item**) nvm_reserve_id("tails", LARGEST_ID * sizeof(item*));
        for(i = 0; i < LARGEST_ID; i++) {
            tails[i] = NULL;
        }
        clwb_range(tails, LARGEST_ID *sizeof(item*));
        sfence();
        nvm_activate_id("tails");

    }
    if (!sizes) {
        sizes = (unsigned int*) nvm_reserve_id("sizes", LARGEST_ID * sizeof(unsigned int));
        for(i = 0; i < LARGEST_ID; i++) {
            sizes[i] = 0;
        }
        clwb_range(sizes, LARGEST_ID *sizeof(item*));
        sfence();
        nvm_activate_id("sizes");
    }
}
*/
...
item *do_item_alloc(char *key, const size_t nkey, const int flags, const rel_time_t exptime, const int nbytes) {
    ...
    pmemobj_zalloc(curr_pool, &item_ptr, ntotal, OBJ_TYPE_NUM);
    it = (item*) pmemobj_direct(item_ptr); 
    if (it == 0) {
        int tries = 50;
        item *search;
...
void item_free(item *it) {
    ...
    DEBUG_REFCNT(it, 'F');
    PMEMoid freed_obj = pmemobj_oid((void*) it);
    pmemobj_free(&freed_obj);
}
