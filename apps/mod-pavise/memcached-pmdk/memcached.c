...
#define LAYOUT_NAME "memcached"
#define POOL_SIZE (512*1024UL*1024UL)

POBJ_LAYOUT_BEGIN(memcached);
POBJ_LAYOUT_ROOT(memcached, item);
//POBJ_LAYOUT_ROOT(memcached, struct lru_lists);
POBJ_LAYOUT_END(memcached);
...
bool BackingFileExists(const char* path) {
    ...
    //strcat(backing_file, "/backing"); //PAVISE_EDIT
    struct stat buffer;
...
int main (int argc, char **argv) {
    ...
    if (!recover) {
        ...
        if ((pop = pmemobj_create(path, POBJ_LAYOUT_NAME(memcached),
                        POOL_SIZE, S_IWUSR | S_IRUSR)) == NULL) {
    ...
    */
    ...
    // stats_init();
    PMEMobjpool* pop = assoc_init(recover, path);
    item_init(pop);
    conn_init();
