		...
		size_t poolsize, mode_t mode)
{
    ...
    //PAVISE_EDIT_BEGIN
    cpavise_create(path, poolsize);

	PMEMobjpool *pop = pmemobj_createU(path, layout, poolsize, mode);
    if(!pop) {LOG(1,"pmemobj_create failed, calling cpavise_delele");cpavise_delete(path);}
	PMEMOBJ_API_END();
	return pop;
}
#else
...
pmemobj_open(const char *path, const char *layout)
{
    ...
    if(!pop) {LOG(1,"pmemobj_openU failed, calling cpavise_close");cpavise_close();}
	PMEMOBJ_API_END();
	return pop;
}
#else
