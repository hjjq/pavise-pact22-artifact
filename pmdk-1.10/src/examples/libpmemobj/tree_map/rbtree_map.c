...
rbtree_map_insert_bst(TOID(struct rbtree_map) map, TOID(struct tree_map_node) n)
{
	...
	TOID(struct tree_map_node) s = D_RO(map)->sentinel;
    //struct tree_map_node tmp; tmp.slots[RB_LEFT] = s; tmp.slots[RB_RIGHT] = s;
	/*memcpy(&D_RW(n)->slots[RB_LEFT], &tmp.slots[RB_LEFT], sizeof(tmp.slots[RB_LEFT])*2);*/D_RW(n)->slots[RB_LEFT] = s;
	...
	//pmemobj_tx_add_range_direct(dst, sizeof(*dst)); //PAVISE_EDIT
	*dst = n;
}
	...
	uint64_t key, PMEMoid value)
{
	...
	TX_BEGIN(pop) {
		TOID(struct tree_map_node) n = TX_ZNEW(struct tree_map_node); //struct tree_map_node tmp; tmp.key=key; tmp.value=value;
		/*memcpy(D_RW(n), &tmp, sizeof(tmp.key) + sizeof(tmp.value));*/D_RW(n)->key = key;
		D_RW(n)->value = value;
