	...
	TOID(struct tree_map_node) left, TOID(struct tree_map_node) right)
{
	...
	} //struct tree_map_node tmp; tmp.slots[p] = left; tmp.slots[p+1] = right; 
	/*printf("executing memcpy, addr = %p\n", &D_RW(node)->slots[p]);memcpy(&D_RW(node)->slots[p], &(tmp.slots[p]), sizeof(tmp.slots[p])*2);*/D_RW(node)->slots[p] = left;
	...
	uint64_t key, int *p)
{
	if (D_RO(n)->n == BTREE_ORDER - 1) { /* node is full, perform a split */
		...
		if (!TOID_IS_NULL(parent)) {
			...
			D_RW(up)->n = 1;
			D_RW(up)->items[0] = m; //struct tree_map_node tmp; tmp.slots[0] = n; tmp.slots[1] = right; 
			/*printf("executing memcpy, addr = %p\n", &D_RW(up)->slots[0]);memcpy(&D_RW(up)->slots[0], &(tmp.slots[0]), sizeof(tmp.slots[0])*2);*/D_RW(up)->slots[0] = n;
