		...
		PMEMoid value, unsigned has_value)
{
	 ...
	 */ //struct tree_map_node* tmp = malloc(sizeof(struct tree_map_node) + key_size); tmp->value = value; tmp->has_value = has_value; tmp->key_size = key_size; memcpy(&tmp->key, key, key_size);
	/*memcpy(&D_RW(node)->has_value, &(tmp->has_value),sizeof(struct tree_map_node) - sizeof(tmp->slots)+ key_size);free(tmp);*/D_RW(node)->value = value;
...
{
	//TX_ADD_FIELD(map, root); //PAVISE_EDIT
	...
	const unsigned char *key, uint64_t key_size, PMEMoid value)
{
	...
	if (TOID_IS_NULL(*node)) {
		//TX_ADD_DIRECT(node); //PAVISE_EDIT
	...
	if (i != D_RO(*node)->key_size) {
		...
		//TX_ADD_FIELD(orig_node, key_size);//PAVISE_EDIT
		D_RW(orig_node)->key_size -= i;

	//	pmemobj_tx_add_range_direct(D_RW(orig_node)->key,
	//			D_RO(orig_node)->key_size); //PAVISE_EDIT
	...
	if (i == key_size) {
		if (OID_IS_NULL(D_RO(*node)->value) || D_RO(*node)->has_value) {
			...
			//TX_ADD_FIELD(*node, has_value);//PAVISE_EDIT
            //struct tree_map_node tmp; tmp.value = value; tmp.has_value = 1;
			/*memcpy(&D_RW(*node)->has_value, &(tmp.has_value),(uintptr_t)&tmp.key_size - (uintptr_t)&tmp.has_value);*/D_RW(*node)->value = value;
