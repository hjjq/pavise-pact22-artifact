	...
	TX_BEGIN(pop) {
		...
		for (size_t i = 0; i < D_RO(buckets_old)->nbuckets; ++i) {
			while (!TOID_IS_NULL(D_RO(buckets_old)->bucket[i])) {
				...
				//TX_ADD_FIELD(en, next); //PAVISE_EDIT
...
{cpavise_tx_begin(); //PAVISE_EDIT
	...
	TX_BEGIN(pop) {
		...
		//TX_ADD_FIELD(hashmap, count); //PAVISE_EDIT
        //tmp = (struct entry) {.key = key, .value=value, .next = D_RO(buckets)->bucket[h]};
		TOID(struct entry) e = TX_NEW(struct entry);//printf("memcpy, &e = %p", D_RW(e));
		/*memcpy(D_RW(e),&tmp,sizeof(struct entry));*/D_RW(e)->key = key;
		D_RW(e)->value = value;
