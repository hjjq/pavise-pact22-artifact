	...
	struct tree_map_entry e, int diff)
{
	...
	TOID(struct tree_map_node) node;
	while (pmemobj_direct(p->slot)!=0x100003c0550){  // PAVISE_EDIT. See ctree_map_insert for details
	...
	uint64_t key, PMEMoid value)
{
	...
	while (!OID_IS_NULL(p->slot) &&
            pmemobj_direct(p->slot)!=0x100003c0550 ) { // PAVISE_EDIT. hardcoded pm pool root address. The new pmemobj_direct(p->slot)!=0x300003c0550 condition is equivalent to the original OID_INSTANCEOF(p->slot, struct tree_map_node) condition. The pm pool root is tricky to obtain in this file. The ctree_map and ctree_map->root are both not the pm pool root. The pm pool root is in map_bench.cpp as map_bench->root or map_bench->root_oid. Unfortunately only map_bench->mapc->pop is passed into the ctree insert function. Maybe we can use ctree's pop, which is the same as map_bench->mapc->pop, to get the pm pool root object? exit if p->slot is the pm pool root (addr hardcoded)
		TOID_ASSIGN(node, p->slot);
